...
```
- The uids are circuitid and name
...
1. Rename circuit name column

2. Check if tibble is Tidy
...
```
- The uids are raceid, driverid and lap
...
1. Left join lap time tibble with tibble join6
...
Pit Stops Data - pit stops done during a safety car and a red flag
...
3. Results/Discussion – It should be a maximum of 3 pages.
...
```{r}
p2 <- join7 %>% filter(deployed ==lap, name =="Singapore Grand Prix") %>% group_by(raceid,deployed) %>%
...
```{r}
p3 <- join7 %>% filter(deployed!=lap, name=="Singapore Grand Prix") %>% group_by(raceid,deployed) %>%
  ggplot(aes(x = as.factor(raceid),y = milliseconds, color = milliseconds)) + 
  geom_boxplot(alpha=.25) + theme_gray() + 
